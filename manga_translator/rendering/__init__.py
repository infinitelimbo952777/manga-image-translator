...
def render(
    ...
    # Warp only the destination bounding box instead of the full image: shift the
    # homography so the warped output is the size of the box, then blend in place.
    ...
    rgba_region = cv2.warpPerspective(box, shift @ M, (x1 - x0, y1 - y0), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
    canvas_region = rgba_region[:, :, :3]
    mask_region = rgba_region[:, :, 3:4].astype(np.float32) / 255.0
    img[y0:y1, x0:x1] = np.clip((img[y0:y1, x0:x1].astype(np.float32) * (1 - mask_region) + canvas_region.astype(np.float32) * mask_region), 0, 255).astype(np.uint8)
    return img
